...
- [ ] Step 8: 教学过程中即时捕获用户的模糊回答、反问、混淆、犹豫、错误迁移和被题目条件卡住的地方，形成阶段级学习证据；不要等章节结束或准备回写时才回忆
- [ ] Step 9: 每个阶段结束时输出“已掌握、模糊/卡点、纠偏方式、待验证边界、当前掌握等级和下一步”，并把需要长期追踪的卡点写入 SQLite misconception/checkpoint
- [ ] Step 10: 交给 `learning-checkpoint` 生成短检验
