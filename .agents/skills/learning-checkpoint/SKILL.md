...
用 1 到 3 个短问题或一个最小实操确认用户是否真正理解，可用面试式追问增强压迫感，但必须控制节奏；同时记录回答中的确定结论、模糊表述、错误因果、遗漏边界、反问和纠偏后的变化，作为阶段学习证据，而不是只记录最终对错。
...
- [ ] Step 5: 先让用户回答，不直接给完整答案；标记回答是明确、部分正确、模糊还是错误，并记录具体证据
- [ ] Step 6: 用户答错或模糊时先补缺口，再最多追问 1 到 2 个相邻问题；记录使用的例子、对比或实验，以及补课后的回答是否改善
- [ ] Step 7: 两轮仍不稳时停止教学检验，标记未掌握，把卡点和证据交给 `concept-teacher`，必要时即时写入 SQLite misconception
## 输出格式
